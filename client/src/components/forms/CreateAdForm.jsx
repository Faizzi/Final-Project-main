...
console.log("id", id);
  ...
  const rapid_api_key = "1223094f4bmsh81020cee8ba47b4p1b0598jsnba0b0aacf285";
  const api_Url = 'https://vehicle-damage-assessment.p.rapidapi.com/run';
  ...
  //   setPreFilledPhone(storedPhone || '');
  // }, []);

  // const handlePhoneChange = (e) => {
  //   // Update the pre-filled phone number as the user types
  //   setPreFilledPhone(e.target.value);
  // };
...
// Inspection Function Rapid Api
const assessVehicleDamage = async (url) => {
  try {
    const options = {
      method: 'POST',
      ...
      headers: {
        ...
        'X-RapidAPI-Key': "1223094f4bmsh81020cee8ba47b4p1b0598jsnba0b0aacf285", // Replace with your Rapid API key
        'X-RapidAPI-Host': 'vehicle-damage-assessment.p.rapidapi.com'
      },
      data: {
        draw_result: false,
        remove_background: false,
        image: url // Use the cover image you have in your formData
      }
    };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await assessVehicleDamage(cover);
 ...
 const handleCoverChanges = (e) => {
  ...
  const handleImagesChange = (e) => {
    ...
    if (selectedFiles.length <= 0) {
      ...
      setFiles([]);
      // Set error message
      setErrorMessageImages('NO IMAGE SELECTED, Maximum of 8 images can be selected');
    } else {
  ...
  const handleSubmitValue = async (event) => {
    ...
    try {
      setPosting(true);
      await newRequest.post(`/posts/postAd/${id}`, formData);
...
// rapid api inspection call
 useEffect(() => {
    if (assessmentResult) {
      if (assessmentResult.output?.elements.length===0) {
        setRating(5);
      }
      else {
        const maxRating = 5;
        const scores = assessmentResult.output?.elements.map(e => e.score);
        const sum = scores.reduce((acc, current) => acc + current, 0);
        const average = sum / scores.length;
        const invertAverage = 1 - average;
        const rating = (invertAverage / 1.0) * maxRating;
        setRating(Math.round(rating * 2) / 2);
...
<div className="col-md-4">
  ...
  <div className="images">
    <div className="imagesInputs">
      <div className="headingss d-flex">
      <label style={{ fontWeight: '600', marginRight:"5px" }} className="form-label" htmlFor="">
        Cover Image</label><p className='textt' style={{color:'gray', fontWeight:"300"}} >*Only 1*</p> </div>
      <input
      ...
      <div style={{ display: 'flex', gap: '10px', marginTop: '5px', marginBottom: '3%', position: 'relative' }}>
          ...
          <div className="headingss d-flex flex-column">
          <div>
                    <span style={{fontSize:"18px"}}><b>Rating:</b></span>
                  <Rating
                    readonly={true}
                    initialValue={rating}
      ...
      <label style={{ fontWeight: '600', marginRight:"5px" }} className="form-label" htmlFor="">
        Upload Images</label><p className='textt' style={{color:'gray', fontWeight:"300"}} >*Maximum 8*</p> </div>
          <input
            className="form-control"
            type="file"
            accept="image/png, image/gif, image/jpeg, image/jpg"
        ...
        <button
          ...
          disabled={!SingleFile || uploaded }
          
        >
          {uploading ? "Uploading" : uploaded ? "Uploaded" :  "Upload Images"}
        </button>
      </div>
    </div>
       {uploaded ? <p>Please Click on POST AD</p> : null}

</div>

    <div className="col-12 pb-5 pt-3">
    <button className="btn btn-primary" type="submit" disabled={!uploadClicked || Posting}>
           { Posting ? "Posting your Ad.." : "POST AD"}
          </button>
  </div>
